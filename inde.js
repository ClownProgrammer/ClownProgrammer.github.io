...
!async function(){
    ...
    document.getElementById(`yes`).onclick = async() => {
        ...
        {space}
        Компьютер и игрок задумывают <b>четырех значное число</b> из различных цифр в диапазоне от 0 до 9. 
        {space}
        ...
        по количеству совпавших чисел и сколько из  них на своем месте
        {space}
        Либо можно просто записать свое число на бумаге, но при этом нужно быть внимательным при ответах,
        не допускать ошибок, иначе компьютер не сможет играть дальше и все прийдется начинать заново
        {space}
        Начинаем играть!
        `,25000)
...
function checkInput(input, moreFunction = () => {return}) {
    let valid_us_numm = 0
...
make_new_move.onclick = async() => {
    ...
    setTimeout(() => {
        if(no_number.checked)
        {
            if(anticipated_number_Us == numberIsPC) {swal(`Ты выиграл, число компьютера ${numberIsPC}`); setTimeout(()=>{location.reload()},3000)}
        }
        else
        {
            if(anticipated_number_PC == numberIsUs && anticipated_number_Us == numberIsPC) {swal(`Ничья, число компьютера ${numberIsPC}`); setTimeout(()=>{location.reload()},3000)}
            if(anticipated_number_PC == numberIsUs) {swal(`Компьютер выиграл, его число ${numberIsPC}`); setTimeout(()=>{location.reload()},3000)}
        }
    }, 500);
}
send.onclick = () => {
    ...
    if(b_pc.innerHTML == 4 && a_pc.innerHTML == 4)
    {
